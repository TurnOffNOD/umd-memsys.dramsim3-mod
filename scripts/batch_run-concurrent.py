...
#       prefix = parse_config.get_val_from_file(c, "other", "output_prefix")
        prefix = pure_config_name
        if args.output_dir:
            prefix = os.path.join(args.output_dir, prefix)
        if args.n:
            prefix_with_cycN = prefix + '-cyc' + str(args.n) # can also use + %s % args.n
            prefix = prefix_with_cycN
        prefix += '/'
        ...
        if not os.path.exists(prefix):
            try:
                os.mkdir(prefix)
                print "WARNING: per-ini result folder not exist, creating one..."
            except OSError:
                print "cannot make directory: ", prefix
                exit(1)
        ...
        cmd_str = "%s %s -c %d --output-dir %s" % \
                  (args.executable, config_file, args.n, prefix)

        
        #if args.cpu_type == "trace":
        if args.trace_file:
            #cmd_str += " --trace-file=%s" % args.trace_file
            cmd_str += " -t %s" % args.trace_file

        
        #print(prefix)
        my_cmd_using = open(os.path.join(prefix,'cmdline.txt'), 'w')
        my_cmd_using.write(cmd_str + '\n')
        my_cmd_using.close
        ...
        try:
            subprocess.Popen(shlex.split(cmd_str), stdout=temp_stdout)
